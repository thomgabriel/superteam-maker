-- C.6 Progressive profile: relax phone_number to nullable.
--
-- Rationale: lowering required fields at signup from 6 to 3 (name, primary_role, years_experience)
-- addresses F3 (36% signup->profile drop). Optional fields collected in /profile/enhance step.
--
-- Migration-safe: empty-string values already work today (NOT NULL rejects NULL, not '').
-- This migration just cleans up by allowing explicit NULL.
--
-- Rollback notes: revert code that passes empty string / null for phone_number; DO NOT
-- re-add NOT NULL (some rows may have empty phone after deploy).

ALTER TABLE profiles ALTER COLUMN phone_number DROP NOT NULL;

-- Update create_profile_and_enter_pool to normalize empty phone to NULL.
CREATE OR REPLACE FUNCTION create_profile_and_enter_pool(
  p_user_id uuid,
  p_name text,
  p_phone_number text,
  p_primary_role text,
  p_macro_role text,
  p_years_experience int,
  p_secondary_roles text[],
  p_interests text[],
  p_linkedin_url text DEFAULT NULL,
  p_github_url text DEFAULT NULL,
  p_x_url text DEFAULT NULL
) RETURNS uuid
LANGUAGE plpgsql
SECURITY DEFINER
SET search_path = public, pg_temp
AS $$
DECLARE
  v_profile_id uuid;
BEGIN
  INSERT INTO profiles (
    user_id, name, phone_number, primary_role, macro_role, years_experience,
    linkedin_url, github_url, x_url
  )
  VALUES (
    p_user_id,
    p_name,
    NULLIF(trim(p_phone_number), ''),
    p_primary_role,
    p_macro_role,
    p_years_experience,
    NULLIF(trim(p_linkedin_url), ''),
    NULLIF(trim(p_github_url), ''),
    NULLIF(trim(p_x_url), '')
  )
  RETURNING id INTO v_profile_id;

  IF array_length(p_secondary_roles, 1) > 0 THEN
    INSERT INTO profile_roles (profile_id, role)
    SELECT v_profile_id, unnest(p_secondary_roles);
  END IF;

  IF array_length(p_interests, 1) > 0 THEN
    INSERT INTO profile_interests (profile_id, interest)
    SELECT v_profile_id, unnest(p_interests);
  END IF;

  INSERT INTO matchmaking_pool (user_id, profile_id, status)
  VALUES (p_user_id, v_profile_id, 'waiting');

  RETURN v_profile_id;
END;
$$;

-- Same for update_profile_details.
CREATE OR REPLACE FUNCTION update_profile_details(
  p_name text,
  p_phone_number text,
  p_primary_role text,
  p_macro_role text,
  p_years_experience int,
  p_secondary_roles text[],
  p_interests text[],
  p_linkedin_url text DEFAULT NULL,
  p_github_url text DEFAULT NULL,
  p_x_url text DEFAULT NULL
)
RETURNS jsonb
LANGUAGE plpgsql
SECURITY DEFINER
SET search_path = public, pg_temp
AS $$
DECLARE
  v_profile_id uuid;
BEGIN
  SELECT id
  INTO v_profile_id
  FROM profiles
  WHERE user_id = auth.uid();

  IF v_profile_id IS NULL THEN
    RETURN jsonb_build_object('success', false, 'code', 'no_profile');
  END IF;

  UPDATE profiles
  SET
    name = p_name,
    phone_number = NULLIF(trim(p_phone_number), ''),
    primary_role = p_primary_role,
    macro_role = p_macro_role,
    years_experience = p_years_experience,
    linkedin_url = NULLIF(trim(p_linkedin_url), ''),
    github_url = NULLIF(trim(p_github_url), ''),
    x_url = NULLIF(trim(p_x_url), '')
  WHERE id = v_profile_id;

  -- Rewrite secondary roles: delete + bulk insert
  DELETE FROM profile_roles WHERE profile_id = v_profile_id;
  IF array_length(p_secondary_roles, 1) > 0 THEN
    INSERT INTO profile_roles (profile_id, role)
    SELECT v_profile_id, unnest(p_secondary_roles);
  END IF;

  -- Rewrite interests: delete + bulk insert
  DELETE FROM profile_interests WHERE profile_id = v_profile_id;
  IF array_length(p_interests, 1) > 0 THEN
    INSERT INTO profile_interests (profile_id, interest)
    SELECT v_profile_id, unnest(p_interests);
  END IF;

  RETURN jsonb_build_object('success', true);
END;
$$;
